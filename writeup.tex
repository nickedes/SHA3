...
\newpage
\newpar
In the backward direction, we can invert from State 4, the known 6 lanes in final state with $\chi^{-1}$. The first five lanes can be directly inverted using $\chi^{-1}$, for the remaining sixth lane, we can fix the seventh lane of State 5 to \textbf{1} for inverting sixth lane. As shown in Fig 3.
